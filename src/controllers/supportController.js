...
export const getUnreadRepliesCount = async (request, response) => {
    try {
        ...
        // Try both aggregation and simple find to ensure it works
        const userTickets = await SupportTicket.find({ userId: userId })
        const count = userTickets.reduce((total, ticket) => total + (ticket.unreadRepliesCount || 0), 0)
