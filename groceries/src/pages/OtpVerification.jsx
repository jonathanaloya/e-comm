    ...
    const handleSubmit = async(e) => {
        ...
        try {
            const response = await Axios({
                ...SummaryApi.forgot_password_otp_vrification,
                data : {
                    otp : data.join(""),
  ...
  return (
    <section className='w-full container mx-auto px-8'>
        <div className='bg-white my-4 w-full max-w-lg mx-auto rounded p-6'>
            <p className='font-semibold text-2xl'>Enter OTP</p>
            <form className='grid gap-4 py-4' onSubmit={handleSubmit}>
                ...
                <button disabled={!validateValue} className={` ${validateValue ? 'bg-green-800 hover:bg-green-600' : 'bg-gray-500'} text-white p-2 rounded font-semibold my-3 tracking-wider`}>Enter OTP</button>
            </form>
